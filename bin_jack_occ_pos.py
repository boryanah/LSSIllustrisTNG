...
if len(sys.argv) > 1:
    if sys.argv[1] == '--help':
        ...
        print('opt = env_cw spin vdisp s2r vani vir moverr nsubs form env_mass vpeak concs concl env shuff conc partial_fenv partial_s2r partial_vani submass')
        exit(1)
...
fix_1bin = True# I think we want true -- fix 1 bin and let the others wander
...
percent_num = 5#tuk#5#35#1 5 10 20
...
if fix_1bin == True:
    fname_shuff = 'figs/many/Corr_poiss_shuff_'+str(st_cut)+'_thr5_'+str(i_bin)+'_'+str(i_bin+skip)+'_pos.txt'
    #figname = 'figs/many/Corr_poiss_'+opt+'_'+str(st_cut)+'_thr'+str(percent_num)+'_'+str(i_bin)+'_'+str(i_bin+skip)+'.png'
    if exclude_most_mass == False: figname = 'figs/paper/Corr_'+opt+'_'+proxy+'_withmostmassive_pos.png'
    else: figname = 'figs/paper/Corr_'+opt+'_'+proxy+'_pos.png'
else:
...
# matching subhalos in fp and dm fp_inds[0] corr to dmo_inds[0] in original Subhalo order
...
# redshift of slice 2.22044604925e-16 and mesh size 128
...
'''
# TUKS
#l_sub_stars_matched = [l_sub_stars[fp_inds] > gal_cuts[i]]]
#SubhaloPos_dm[dmo_inds] == SubhaloPos_fp[fp_inds]
pos_sub_matched_dm = SubhaloPos_dm[dmo_inds]
pos_gal_matched_dm = pos_sub_matched_dm[l_sub_stars[fp_inds] > gal_cuts[i]]]
inds_gal_matched_dm = inds_sub_dm[l_sub_stars[fp_inds] > gal_cuts[i]]
par_gal_matched_dm = parent_dm[l_sub_stars[fp_inds] > gal_cuts[i]]
'''
...
#SubhaloPos_fp[GroupFirstSub_fp[count_halo_fp > 0.5]]
'''
# I think
print(count_halo_fp[count_halo_fp > 0.5])
print(counts)
print(count_halo_fp[count_halo_fp > 0.5].shape)
print(counts.shape)
quit()tuks
'''
...
#conc_halo_dm_sorted = count_halo_dm_sorted
...
if fix_1bin == True:
    ...
    print("sum halo dm shuff = ",np.sum(count_halo_dm_shuff))
...
bin_centers, a,b,c,d,xi_fid_shuff,xi_fid_shuff_err,rat_fid_shuff,rat_fid_shuff_err = np.loadtxt('/home/boryanah/lars/test/figs/paper/Corr_shuff_'+proxy+'_pos.txt',unpack=True)
...
'''
# REMOVE for new proxies (I think with exclusion)
np.savetxt('/home/boryanah/lars/test/figs/paper/Corr_shuff_'+proxy+'_pos.txt',(np.vstack((bin_centers, Corr_mean_FP*bin_centers**2,Corr_err_FP*bin_centers**2,Corr_mean_DM*bin_centers**2,Corr_err_DM*bin_centers**2,Corr_mean_DM_shuff*bin_centers**2,Corr_err_DM_shuff*bin_centers**2,Rat_HOD_mean,Rat_HOD_err)).T))
quit()
'''
