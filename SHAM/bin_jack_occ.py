...
isolate_low_mass = 0#1#only for splits and not for mstar split
if len(sys.argv) > 4:
    sec_prop = sys.argv[4]
...
SubhaloVpeak_dm = np.load(root+'SubhaloVpeak_'+dm_ext+'.npy')
SubhaloVpeak_dm[SubhaloVpeak_dm==0.] = SubhaloVmax_dm[SubhaloVpeak_dm==0.]
#SubhaloMpeak_dm = np.load(root+'SubhaloMpeak_dm_60000.npy')
#SubhaloMpeak_dm[SubhaloMpeak_dm==0.] = SubhaloMmax_dm[SubhaloMpeak_dm==0.]
SubhaloVinfall_dm = np.load(root+'SubhaloVinfall_'+dm_ext+'.npy')
SubhaloVrelax_dm = np.load(root+'SubhaloVrelax_'+dm_ext+'.npy')
...
SubhaloMpeak_dm = np.load(root+'SubhaloMpeak_'+dm_ext+'.npy')
SubhaloMpeak_dm[SubhaloMpeak_dm==0.] = SubhaloMassType_dm[SubhaloMpeak_dm==0.,1]
SubhaloMinfall_dm = np.load(root+'SubhaloMinfall_'+dm_ext+'.npy')
...
if isolate_low_mass:
    ...
    prop_choice = (SubhaloMassType_fp[:,4] < 5.) & (SubhaloMassType_fp[:,4] > 1.);
    #prop_choice = (SubhaloMassType_fp[:,4] > 3.);
...
if sec_prop != 'full':
    ...
    if sec_prop == 'mstar':
        prop_fp = (SubhaloMassType_fp[:,4]*1.e10)
        M_lim = 5.#3. #10^10 Msun/h
        if i_bin == 0: text = "low stellar mass"; prop_choice = (SubhaloMassType_fp[:,4] < M_lim) & (SubhaloMassType_fp[:,4] > 1.);
        if i_bin == 1: text = "high stellar mass"; prop_choice = (SubhaloMassType_fp[:,4] > M_lim);
    elif sec_prop == 'g-r':
        prop_fp = gmr_fp
        gr_lim = .6#.73#0.7
...
if N_div == 1:
    np.save("data/SHAM"+want_matched+"_ratio_"+proxy+".npy",Rat_SHAM_mean)
    np.save("data/SHAM"+want_matched+"_ratio_"+proxy+"_error.npy",Rat_SHAM_err)
